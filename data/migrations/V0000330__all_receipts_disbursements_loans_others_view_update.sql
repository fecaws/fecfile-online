 ...
 SELECT sa.cmte_id,
    ...
    rt.category_type,
        ...
        CASE
            ...
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = sa.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
        CASE
            WHEN sa.transaction_type_identifier::text = ANY (ARRAY['TRIB_REC'::character varying::text, 'IK_REC'::character varying::text, 'IK_BC_REC'::character varying::text, 'PARTN_MEMO'::character varying::text, 'BC_TO_IND_MEMO'::character varying::text, 'BC_TO_UNKN_MEMO'::character varying::text, 'INDV_REC'::character varying::text, 'PARTN_REC'::character varying::text, 'REATT_FROM'::character varying::text, 'REATT_TO'::character varying::text, 'RET_REC'::character varying::text, 'BC_TO_IND'::character varying::text, 'BC_TO_UNKN'::character varying::text, 'IND_NP_HQ_ACC'::character varying::text, 'TRIB_NP_HQ_ACC'::character varying::text, 'IND_NP_CONVEN_ACC'::character varying::text, 'TRIB_NP_CONVEN_ACC'::character varying::text, 'EAR_REC_RECNT_ACC'::character varying::text, 'EAR_REC_CONVEN_ACC'::character varying::text, 'EAR_REC_HQ_ACC'::character varying::text, 'IND_REC_NON_CONT_ACC'::character varying::text, 'BUS_LAB_NON_CONT_ACC'::character varying::text, 'OFFSET_TO_OPEX'::character varying::text, 'TRIB_NP_RECNT_ACC'::character varying::text, 'IND_NP_RECNT_ACC'::character varying::text, 'IND_RECNT_REC'::character varying::text, 'OTH_REC'::character varying::text, 'OTH_REC_DEBT'::character varying::text]) THEN true
            ELSE false
 ...
 SELECT sb.cmte_id,
    ...
    sb.beneficiary_cmte_id,
        ...
        CASE
            ...
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = sb.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
        CASE
            WHEN sb.transaction_type_identifier::text = ANY (ARRAY['OPEXP'::character varying::text, 'OPEXP_CC_PAY'::character varying::text, 'OPEXP_STAF_REIM'::character varying::text, 'OPEXP_PMT_TO_PROL'::character varying::text, 'OPEXP_VOID'::character varying::text, 'OPEXP_HQ_ACC_OP_EXP_NP'::character varying::text, 'OPEXP_CONV_ACC_OP_EXP_NP'::character varying::text, 'OPEXP_DEBT'::character varying::text, 'OTH_DISB'::character varying::text, 'OTH_DISB_CC_PAY'::character varying::text, 'OTH_DISB_STAF_REIM'::character varying::text, 'OTH_DISB_PMT_TO_PROL'::character varying::text, 'OTH_DISB_RECNT'::character varying::text, 'OTH_DISB_NP_RECNT_ACC'::character varying::text, 'OTH_DISB_NC_ACC'::character varying::text, 'OTH_DISB_NC_ACC_CC_PAY'::character varying::text, 'OTH_DISB_NC_ACC_STAF_REIM'::character varying::text, 'OTH_DISB_NC_ACC_PMT_TO_PROL'::character varying::text, 'OTH_DISB_VOID'::character varying::text, 'REF_CONT_IND'::character varying::text, 'REF_CONT_IND_VOID'::character varying::text, 'FEA_100PCT_PAY'::character varying::text, 'FEA_CC_PAY'::character varying::text, 'FEA_STAF_REIM'::character varying::text, 'FEA_PAY_TO_PROL'::character varying::text, 'FEA_VOID'::character varying::text, 'FEA_100PCT_DEBT_PAY'::character varying::text]) THEN true
            ELSE false
        END AS forceitemizable,
        CASE
            WHEN (( SELECT count(cs.transaction_id) AS count
 ...
 SELECT se.cmte_id,
    ...
    NULL::character varying AS beneficiary_cmte_id,
        ...
        CASE
            ...
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = se.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
    NULL::boolean AS forceitemizable,
        CASE
            WHEN (( SELECT count(cs.transaction_id) AS count
               FROM all_transactions_view cs
 ...
 SELECT sf.cmte_id,
    ...
    NULL::character varying AS beneficiary_cmte_id,
        ...
        CASE
            ...
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = sf.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
 ...
 SELECT sc.cmte_id,
    ...
    rt.category_type,
        ...
        CASE
            ...
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = sc.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
 ...
 SELECT sd.cmte_id,
    ...
    rt.category_type,
        ...
        CASE
            ...
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = sd.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
...
UNION
 SELECT sb.cmte_id,
    ...
    NULL::character varying AS ratio_code,
        ...
        END AS iseditable,
        CASE
            WHEN sb.transaction_type_identifier::text = ANY (ARRAY['LEVIN_OTH_DISB'::character varying::text]) THEN true
            ELSE false
        END AS forceitemizable,
        CASE
            WHEN (( SELECT count(cs.transaction_id) AS count
 ...
 SELECT h1.cmte_id,
    ...
    NULL::character varying AS ratio_code,
        CASE
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = h1.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
 ...
 SELECT h2.cmte_id,
    ...
    h2.ratio_code,
        CASE
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = h2.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
 ...
 SELECT h3.cmte_id,
    ...
    NULL::character varying AS ratio_code,
        CASE
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = h3.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
 ...
 SELECT h4.cmte_id,
    ...
    NULL::character varying AS ratio_code,
        CASE
            ...
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = h4.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
 ...
 SELECT h5.cmte_id,
    ...
    NULL::character varying AS ratio_code,
        CASE
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = h5.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
 ...
 SELECT h6.cmte_id,
    ...
    NULL::character varying AS ratio_code,
        CASE
            ...
            WHEN rp.delete_ind = 'Y'::bpchar THEN false
            WHEN (SELECT (CASE WHEN crp.status IS NULL OR crp.status = 'Saved' THEN 'Saved' ELSE 'Filed' END) 
                FROM reports crp WHERE crp.cmte_id = h6.cmte_id AND crp.cvg_start_date = rp.cvg_start_date AND 
                crp.delete_ind IS DISTINCT FROM 'Y'::bpchar
                ORDER BY crp.amend_ind ASC, crp.amend_number DESC LIMIT 1) = 'Filed' THEN false
            ELSE true
        END AS iseditable,
