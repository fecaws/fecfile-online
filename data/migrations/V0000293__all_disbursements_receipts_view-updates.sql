ALTER TABLE public.sched_b ADD COLUMN redesignation_id character varying(20),
ADD COLUMN redesignation_ind character varying(1);
...
CREATE OR REPLACE VIEW public.all_receipts_transactions_view AS 
 SELECT sa.cmte_id,
    ...
    rt.category_type,
        ...
        END AS haschild,
        CASE
		    WHEN (sa.transaction_type_identifier::text = ANY (ARRAY['IND_REC_NON_CONT_ACC'::character varying::text, 
		    	'IND_NP_CONVEN_ACC'::character varying::text, 'IND_NP_HQ_ACC'::character varying::text, 
		    	'IND_NP_RECNT_ACC'::character varying::text, 'IND_RECNT_REC'::character varying::text, 
		    	'INDV_REC'::character varying::text, 'JF_TRAN_IND_MEMO'::character varying::text, 
		    	'JF_TRAN_NP_RECNT_IND_MEMO'::character varying::text,
		    	'JF_TRAN_NP_CONVEN_IND_MEMO'::character varying::text,
		    	'JF_TRAN_NP_HQ_IND_MEMO'::character varying::text])) 
		    AND sa.reattribution_ind IS NULL THEN true
		    ELSE false
...
CREATE OR REPLACE VIEW public.all_disbursements_transactions_view AS 
 SELECT sb.cmte_id,
    ...
    sb.beneficiary_cmte_id,
        ...
        END AS haschild,
        CASE
		    WHEN sb.transaction_type_identifier::text = 'CONT_TO_CAN'
		    AND sb.redesignation_ind IS NULL THEN true
		    ELSE false
