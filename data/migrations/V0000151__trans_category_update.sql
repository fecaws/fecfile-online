alter table transaction_categories add column is_functional boolean default true;
                 ...
                 WITH sub_category AS (
                         SELECT tc.form_type,
                            ...
                            ( SELECT json_agg(json_build_object('type', 'radio', 'text', trc.tran_type, 'name', regexp_replace(lower(trc.sub_category_type::text), '[^a-zA-Z0-9]'::text, '-'::text, 'g'::text), 'value', trc.tran_type_identifier, 'infoIcon', trc.tran_type_info_icon, 'info', trc.tran_type_info) ORDER BY trc.tran_type_order) AS json_agg
                                  ...
                                  WHERE trc.form_type::text = tc.form_type::text AND trc.cmte_type_category::text = tc.cmte_type_category::text AND trc.category_type::text = tc.category_type::text AND trc.sub_category_type::text = tc.sub_category_type::text AND trc.tran_type IS NOT NULL and is_functional=true) AS json_by_tran
