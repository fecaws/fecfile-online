...
UNION
 SELECT h1.cmte_id,
    ...
    false AS haschild,
        CASE
            WHEN ((SELECT cmte_type_category FROM public.committee_master cm WHERE cm.cmte_id = h1.cmte_id) = 'PAC') AND
            ((h1.administrative = 't' AND ((SELECT count(ch3.transaction_id) FROM sched_h3 ch3 WHERE ch3.activity_event_type = 'AD' AND ch3.cmte_id = h1.cmte_id AND date_part('year', ch3.receipt_date) = h1.election_year AND ch3.delete_ind is distinct from 'Y')+
                (SELECT count(ch4.transaction_id) FROM sched_h4 ch4 WHERE ch4.activity_event_type = 'AD' AND ch4.cmte_id = h1.cmte_id AND date_part('year', ch4.expenditure_date) = h1.election_year AND ch4.delete_ind is distinct from 'Y')) > 0) OR
                (h1.generic_voter_drive = 't' AND ((SELECT count(ch3.transaction_id) FROM sched_h3 ch3 WHERE ch3.activity_event_type = 'GV' AND ch3.cmte_id = h1.cmte_id AND date_part('year', ch3.receipt_date) = h1.election_year AND ch3.delete_ind is distinct from 'Y')+
                (SELECT count(ch4.transaction_id) FROM sched_h4 ch4 WHERE ch4.activity_event_type = 'GV' AND ch4.cmte_id = h1.cmte_id AND date_part('year', ch4.expenditure_date) = h1.election_year AND ch4.delete_ind is distinct from 'Y')) > 0) OR
                (h1.public_communications = 't' AND ((SELECT count(ch3.transaction_id) FROM sched_h3 ch3 WHERE ch3.activity_event_type = 'PC' AND ch3.cmte_id = h1.cmte_id AND date_part('year', ch3.receipt_date) = h1.election_year AND ch3.delete_ind is distinct from 'Y')+
                (SELECT count(ch4.transaction_id) FROM sched_h4 ch4 WHERE ch4.activity_event_type = 'PC' AND ch4.cmte_id = h1.cmte_id AND date_part('year', ch4.expenditure_date) = h1.election_year AND ch4.delete_ind is distinct from 'Y')) > 0)
                ) THEN false
            WHEN (((SELECT cmte_type_category FROM public.committee_master cm WHERE cm.cmte_id = h1.cmte_id) = 'PTY') AND
            ((SELECT count(ch3.transaction_id) FROM sched_h3 ch3 WHERE ch3.cmte_id = h1.cmte_id AND date_part('year', ch3.receipt_date) = h1.election_year AND ch3.delete_ind is distinct from 'Y') +
             (SELECT count(ch4.transaction_id) FROM sched_h4 ch4 WHERE ch4.cmte_id = h1.cmte_id AND date_part('year', ch4.expenditure_date) = h1.election_year AND ch4.delete_ind is distinct from 'Y') +
             (SELECT count(ch5.transaction_id) FROM sched_h5 ch5 WHERE ch5.cmte_id = h1.cmte_id AND date_part('year', ch5.receipt_date) = h1.election_year AND ch5.delete_ind is distinct from 'Y') +
             (SELECT count(ch6.transaction_id) FROM sched_h6 ch6 WHERE ch6.cmte_id = h1.cmte_id AND date_part('year', ch6.expenditure_date) = h1.election_year AND ch6.delete_ind is distinct from 'Y')) > 0)
              THEN false
            ELSE true
        END AS istrashable
   FROM sched_h1 h1
...
UNION
 SELECT h2.cmte_id,
    ...
    false AS haschild,
        CASE
            WHEN (((SELECT count(ch3.transaction_id) FROM sched_h3 ch3 WHERE ch3.activity_event_name = h2.activity_event_name AND ch3.cmte_id = h2.cmte_id AND ch3.delete_ind is distinct from 'Y')+
                (SELECT count(ch4.transaction_id) FROM sched_h4 ch4 WHERE ch4.activity_event_identifier = h2.activity_event_name AND ch4.cmte_id = h2.cmte_id AND ch4.delete_ind is distinct from 'Y')) > 0)
                 THEN false
            ELSE true
        END AS istrashable
   FROM sched_h2 h2
