DELETE FROM public.dynamic_form_fields WHERE transaction_type in ('FEA_100PCT_DEBT_PAY','COEXP_PARTY_DEBT','OTH_DISB_DEBT','ALLOC_EXP_DEBT','ALLOC_FEA_DISB_DEBT','OPEXP_DEBT') AND field_db_name='memo_code';
...
INSERT INTO public.dynamic_form_fields(
  ...
  SELECT form_type, 'COEXP_PARTY_DEBT', field_label, field_value, 
       ...
       seperator, transaction_type_desc, field_input_group, field_input_icon, 
       child_form, form_sub_title, toggle, entity_group, entity_id_mapping
  FROM public.dynamic_form_fields WHERE transaction_type='FEA_100PCT_DEBT_PAY' AND field_db_name='beneficiary_cand_entity_id';

UPDATE ref_transaction_types SET ref_tran_id='10111' WHERE ref_tran_id='10068';
UPDATE ref_transaction_types SET ref_tran_id='10068' WHERE ref_tran_id='10042';
UPDATE ref_transaction_types SET ref_tran_id='10042' WHERE ref_tran_id='10064';
UPDATE ref_transaction_types SET ref_tran_id='10064' WHERE ref_tran_id='10061';
UPDATE ref_transaction_types SET ref_tran_id='10061' WHERE ref_tran_id='10069';
UPDATE ref_transaction_types SET ref_tran_id='10069' WHERE ref_tran_id='10070';
UPDATE ref_transaction_types SET ref_tran_id='10070' WHERE ref_tran_id='10111';
          ...
          WHERE dfe.form_type::text = dfbs.form_type::text AND dfe.transaction_type::text = dfbs.transaction_type::text), 'electionTypes', ( SELECT json_agg(json_build_object('electionType', ref_election_type.election_type, 'electionTypeDescription', ref_election_type.election_type_desc)) AS json_agg
           ...
           FROM ( SELECT 'PAC'::text AS "committeeTypeCategory",
                    json_agg(json_build_object('eventType', ref_event_types.event_type, 'eventTypeDescription', ref_event_types.event_type_desc, 'scheduleType',
                        CASE
                            WHEN ref_event_types.event_type::text = ANY (ARRAY['DF'::character varying, 'DC'::character varying]::text[]) THEN 'sched_h2'::text
                            ELSE 'sched_h1'::text
