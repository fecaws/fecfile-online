...
ALTER TABLE public.sched_a
    ALTER COLUMN itemized_ind TYPE character varying(2);

ALTER TABLE public.sched_b
    ALTER COLUMN itemized_ind TYPE character varying(2);


-- View: public.all_transactions_view

-- DROP VIEW public.all_transactions_view;

CREATE OR REPLACE VIEW public.all_transactions_view
 AS
 SELECT sa.cmte_id,
    sa.report_id,
 ...
 SELECT sa.cmte_id,
    ...
    rt.category_type,
        ...
        CASE
            ...
            WHEN sa.reattribution_ind = 'A' THEN false
            WHEN sa.reattribution_ind = 'R' AND ((SELECT csa1.delete_ind FROM sched_a csa1 WHERE csa1.transaction_id = sa.reattribution_id) = 'Y') THEN false
            WHEN sa.reattribution_ind = 'R' AND sa.delete_ind = 'Y' AND ((SELECT csa2.reattribution_ind FROM sched_a csa2 WHERE csa2.transaction_id = sa.reattribution_id) is NOT NULL) THEN false
            ELSE true
        END AS iseditable,
        CASE
        ...
        CASE
            WHEN sa.reattribution_ind = 'R' THEN true
            ELSE false
...
CREATE OR REPLACE VIEW public.all_other_transactions_view
 AS
 SELECT sa.cmte_id,
    sa.report_id,
    rp.report_type,
...
CREATE OR REPLACE VIEW public.all_loans_debts_transactions_view
 AS
 SELECT sc.cmte_id,
    sc.report_id,
    rp.report_type,
...
CREATE OR REPLACE VIEW public.all_contacts_view
 AS
 SELECT e.cmte_id,
    e.entity_id AS id,
        CASE
