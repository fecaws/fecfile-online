CREATE OR REPLACE VIEW public.transaction_subcategory_json_view AS
 WITH tran_sub_category AS (
         SELECT DISTINCT trc.form_type,
            trc.category_type,
            trc.sub_category_type,
            trc.sub_category_type_info,
            trc.category_order,
            trc.sub_category_order
           FROM transaction_categories trc
          WHERE trc.form_type::text = 'F3X'::text
          ORDER BY trc.category_order, trc.sub_category_order
        )
 SELECT tran_sub_category.form_type,
    json_agg(json_build_object('value', lower(replace(tran_sub_category.sub_category_type::text, ' '::text, '-'::text)), 'text', tran_sub_category.sub_category_type, 'type', tran_sub_category.category_type, 'info', tran_sub_category.sub_category_type_info) ORDER BY tran_sub_category.category_order, tran_sub_category.sub_category_order) AS tran_subcategory
   FROM tran_sub_category
  GROUP BY tran_sub_category.form_type;
         ...
         WITH categories AS (
                 WITH sub_category AS (
                         SELECT tc.form_type,
                            ...
                            ( SELECT json_agg(json_build_object('type', 'radio', 'text', trc.tran_type, 'name', regexp_replace(lower(trc.sub_category_type::text), '[^a-zA-Z0-9]'::text, '-'::text, 'g'::text), 'value', regexp_replace(lower(trc.tran_type::text), '[^a-zA-Z0-9]'::text, '-'::text, 'g'::text), 'infoIcon', trc.tran_type_info_icon, 'info', trc.tran_type_info, 'tranTypeIdentifier', trc.tran_type_identifier) ORDER BY trc.tran_type_order) AS json_agg
                                   FROM transaction_categories trc
                                  WHERE trc.form_type::text = tc.form_type::text AND trc.category_type::text = tc.category_type::text AND trc.sub_category_type::text = tc.sub_category_type::text AND trc.tran_type IS NOT NULL) AS json_by_tran
                 ...
                 SELECT sc.form_type,
                    ...
                    json_agg(json_build_object('text', sc.sub_category_type, 'name', regexp_replace(lower(sc.sub_category_type::text), '[^a-zA-Z0-9]'::text, '-'::text, 'g'::text), 'value', regexp_replace(lower(sc.sub_category_type::text), '[^a-zA-Z0-9]'::text, '-'::text, 'g'::text), 'infoIcon', ( SELECT tc.sub_category_type_info_icon
                           FROM transaction_categories tc
                          WHERE tc.form_type::text = sc.form_type::text AND tc.category_type::text = sc.category_type::text AND tc.sub_category_type::text = sc.sub_category_type::text
