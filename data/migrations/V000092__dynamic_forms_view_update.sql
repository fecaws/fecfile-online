ALTER TABLE dynamic_form_fields ADD field_input_group boolean NOT NULL DEFAULT 'false';
ALTER TABLE dynamic_form_fields ADD field_input_icon character varying(30) NULL DEFAULT '';
...
CREATE OR REPLACE VIEW public.dynamic_forms_view AS 
 ...
 SELECT dfbs.form_type,
    ...
    json_build_object('data', json_build_object('formFields', json_agg(json_build_object('colClassName', dfbs.class_name, 'seperator', dfbs.seperator, 'cols', dfbs.json_by_section) ORDER BY dfbs.field_section_order), 'hiddenFields', ( SELECT json_agg(json_build_object('type', h.field_type, 'name', replace(h.field_db_name::text, ' '::text, ''::text), 'value', h.field_value)) AS json_agg
           FROM dynamic_form_fields h
          WHERE h.field_type::text = 'hidden'::text AND h.form_type::text = dfbs.form_type::text AND h.transaction_type_identifier::text = dfbs.transaction_type_identifier::text), 'states', ( SELECT json_agg(json_build_object('name', ref_states.state_description, 'code', ref_states.state_code) ORDER BY ref_states.st_number) AS json_agg
          ...
          WHERE dft.form_type::text = dfbs.form_type::text AND dft.tran_type_identifier::text = dfbs.transaction_type_identifier::text))) AS form_fields
