...
UNION
 SELECT h1.cmte_id,
    ...
    false AS haschild,
        CASE
            WHEN ((h1.administrative = 't' AND ((SELECT count(ch3.transaction_id) FROM sched_h3 ch3 WHERE ch3.activity_event_type = 'AD' AND ch3.cmte_id = h1.cmte_id AND date_part('year', ch3.receipt_date) = h1.election_year AND ch3.delete_ind is distinct from 'Y')+
...
UNION
 SELECT h2.cmte_id,
    ...
    false AS haschild,
        CASE
            WHEN (((SELECT count(ch5.transaction_id) FROM sched_h5 ch5 WHERE ch5.account_name = h2.activity_event_name AND ch5.cmte_id = h2.cmte_id AND ch5.delete_ind is distinct from 'Y')+
                (SELECT count(ch6.transaction_id) FROM sched_h6 ch6 WHERE ch6.account_event_identifier = h2.activity_event_name AND ch6.cmte_id = h2.cmte_id AND ch6.delete_ind is distinct from 'Y')) > 0)
                 THEN false
            ELSE true
        END AS istrashable
   FROM sched_h2 h2
