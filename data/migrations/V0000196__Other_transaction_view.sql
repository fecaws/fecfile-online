...
CREATE OR REPLACE VIEW public.all_other_transactions_view AS 
 SELECT sa.cmte_id,
    ...
    NULL AS activity_event_identifier,
    rt.category_type,
        CASE
   ...
   FROM sched_a sa 
    ...
    WHERE sa.transaction_type_identifier in ('LEVIN_PARTN_MEMO', 'LEVIN_TRIB_REC', 'LEVIN_PARTN_REC', 'LEVIN_ORG_REC', 'LEVIN_INDV_REC', 'LEVIN_NON_FED_REC', 'LEVIN_OTH_REC', 'LEVIN_PAC_REC')
...
UNION
 SELECT sb.cmte_id,
   ...
   FROM sched_b sb 
     ...
     JOIN ref_rpt_types rrt ON rp.report_type::text = rrt.rpt_type::text
    WHERE sb.transaction_type_identifier in ('LEVIN_VOTER_REG', 'LEVIN_VOTER_ID', 'LEVIN_GOTV', 'LEVIN_GEN','LEVIN_OTH_DISB')
...
UNION
 SELECT h5.cmte_id, 
        ...
        total_amount_transferred AS transaction_amount,
        NULL AS aggregate_amt,
        NULL AS purpose_description,
        h5.memo_code, 
        h5.memo_text,  
