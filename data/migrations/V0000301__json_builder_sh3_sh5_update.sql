...
VALUES ('F3X', 'SH5', 'TRAN_FROM_LEVIN_ACC', '
                    ...
                    WHERE t1.transaction_type_identifier = ''TRAN_FROM_LEVIN_ACC'' AND t1.report_id = %s AND t1.cmte_id = %s 
                    AND t1.back_ref_transaction_id is not null AND t1.delete_ind is distinct from ''Y''
                    ');
...
VALUES ('F3X', 'SH3', 'TRAN_FROM_NON_FED_ACC', '
                    ...
                    WHERE t1.transaction_type_identifier = ''TRAN_FROM_NON_FED_ACC'' AND t1.report_id = %s AND t1.cmte_id = %s 
                    AND t1.back_ref_transaction_id is not null AND t1.delete_ind is distinct from ''Y''
                    ');
