...
CREATE OR REPLACE VIEW public.reports_view AS 
 SELECT reports.report_id,
    ...
    reports.previous_report_id,
           CASE
            WHEN reports.status = 'Submitted' THEN 'Filed'
            WHEN reports.status = 'Saved' THEN 'Saved'
            WHEN reports.status = 'Failed'  THEN 'Failed'
            ELSE NULL
        END AS status,
    reports.filed_date,
    reports.fec_id,
...
UNION
 SELECT forms_committeeinfo.id AS report_id,
    ...
    NULL::bigint AS previous_report_id,
        CASE
            WHEN forms_committeeinfo.is_submitted = true THEN 'Filed'
            WHEN forms_committeeinfo.is_submitted = false THEN 'Saved'
            WHEN forms_committeeinfo.is_failed = true THEN 'Failed'
            ELSE NULL
        END AS status,
        CASE
