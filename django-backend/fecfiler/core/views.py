...
def post_sql_form3x(
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                """INSERT INTO public.form_3x (report_id, cmte_id, form_type, amend_ind, report_type, election_code, date_of_election, state_of_election, cvg_start_dt, cvg_end_dt, coh_bop)
...
def get_trans_query(category_type, cmte_id, param_string):
    ...
    if category_type == "disbursements_tran":
        ...
        query_string = (
            """SELECT report_id, report_type, reportStatus, transaction_type, transaction_type_desc, transaction_id, api_call, name, street_1, street_2, city, state, zip_code, occupation, employer, 
        ...
        query_string = (
            """SELECT report_id, schedule, report_type, reportStatus, activity_event_identifier, transaction_type, transaction_type_desc, transaction_id, back_ref_transaction_id, api_call, 
...
def period_disbursements_for_summary_table_sql( calendar_start_dt, calendar_end_dt, cmte_id, report_id):
    ...
    """
    logger.debug('loading period_dsibursements for calendar start {}, end {}, report_id {}'.format(calendar_start_dt, calendar_end_dt, report_id))
    rep_sql = """
...
def get_reporttype(cmte_id, report_id):
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """SELECT report_type FROM public.reports WHERE cmte_id = %s AND report_id = %s """,
                [cmte_id, report_id],
            )
            data_row = []
            report_type = ""
...
def get_report_ids(cmte_id, from_date):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """SELECT report_id FROM public.reports WHERE cmte_id= %s AND cvg_start_date >= %s AND status = 'Submitted' AND form_type = 'F3X' AND superceded_report_id IS NULL""",
                [cmte_id, from_date],
            )
            if cursor.rowcount > 0:
                for row in cursor.fetchall():
